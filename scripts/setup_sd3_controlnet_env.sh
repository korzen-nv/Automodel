...
DIFFUSERS_DIR=${1:?usage: setup_sd3_controlnet_env.sh DIFFUSERS_DIR VENV_DIR [REF] [BASE_ENV_PYTHON]}
VENV_DIR=${2:?usage: setup_sd3_controlnet_env.sh DIFFUSERS_DIR VENV_DIR [REF] [BASE_ENV_PYTHON]}
DIFFUSERS_REF=${3:-main}
BASE_ENV_PYTHON=${4:-}
SENTINEL="${VENV_DIR}/.sd3_controlnet_env_ready"
META_FILE="${VENV_DIR}/.sd3_controlnet_env_meta"
...
# SD3 ControlNet uses requirements_sd3.txt if it exists, otherwise requirements.txt
if [ -f "${DIFFUSERS_DIR}/examples/controlnet/requirements_sd3.txt" ]; then
    REQUIREMENTS_FILE="${DIFFUSERS_DIR}/examples/controlnet/requirements_sd3.txt"
else
    REQUIREMENTS_FILE="${DIFFUSERS_DIR}/examples/controlnet/requirements.txt"
fi
...
if [ -f "${SENTINEL}" ] && [ -f "${META_FILE}" ] && [ "$(cat "${META_FILE}")" = "${EXPECTED_META}" ]; then
    echo "SD3 ControlNet environment already prepared at ${VENV_DIR}"
    exit 0
fi
...
touch "${SENTINEL}"
echo "Prepared SD3 ControlNet environment at ${VENV_DIR}"
